...
def update_student(sno,sname,sage,sdob,scourse,course_duration):
    ...
    for stud in data["students"]:
        if stud["sno"]==int(sno):
            ...
            json_1.write_json("data/user.json",data)
...
def index():
    if request.method=="POST":
        register_student(request.form["sno"],request.form["studname"],request.form["studage"],request.form["dob"],request.form["course"],request.form["duration"])
